import React from 'react'
import { Footer } from "./Footer";
import "../index.css";

 const Contact = () => {
  return (
    <>
    <main>


     <section className="w-full  px-6 py-24 border-y border-slate-300 flex flex-col lg:flex-row gap-16">
      <div className='w-full'>
          <p className="text-xs tracking-widest text-slate-600">CONTACT</p>

          <h2 className="mt-6 text-5xl lg:text-7xl font-semibold leading-tight tracking-tight text-slate-900">
            Creating true <br /> brands together.
          </h2>

          {/* Socials */}
          <div className="mt-12">
            <p className="text-xs tracking-widest text-slate-600">
              FIND US ON
            </p>

            <div className="mt-4 flex gap-3">
              {["IN", "PI", "TW"].map((item) => (
                <span
                key={item}
                className="w-10 h-10 flex items-center justify-center border border-slate-500 rounded-full text-xs hover:bg-black hover:text-white transition"
                >
                  {item}
                </span>
              ))}
            </div>
          </div>
          </div>

<div className='w-full'>

        {/* Right Side Form */}
        <form className="space-y-10">
          {/* Name */}
          <div className="relative">
            <input
              type="text"
              placeholder=" "
              className="peer w-full bg-transparent border-b border-slate-500 py-3 outline-none"
            />
            <label className="absolute left-0 top-3 text-slate-500 text-sm transition-all peer-focus:-top-4 peer-focus:text-xs peer-focus:text-black peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm">
              Your Name
            </label>
          </div>

          {/* Email */}
          <div className="relative">
            <input
              type="email"
              placeholder=" "
              className="peer w-full bg-transparent border-b border-slate-500 py-3 outline-none"
            />
            <label className="absolute left-0 top-3 text-slate-500 text-sm transition-all peer-focus:-top-4 peer-focus:text-xs peer-focus:text-black peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm">
              Your Email
            </label>
          </div>

          {/* Message */}
          <div className="relative">
            <textarea
              rows="4"
              placeholder=" "
              className="peer w-full bg-transparent border-b border-slate-500 py-3 outline-none resize-none"
              />
            <label className="absolute left-0 top-3 text-slate-500 text-sm transition-all peer-focus:-top-4 peer-focus:text-xs peer-focus:text-black peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm">
              Your Message
            </label>
          </div>

          {/* Button */}
          <button
            type="submit"
            className="w-full bg-black text-white py-4 rounded-lg tracking-widest text-sm hover:bg-slate-800 transition"
            >
            FILL OUT THE FORM
          </button>
        </form>
            </div>
    </section>
    <br /><hr /><br />
     <section className="view-3">
            <div className="left">
                <h1>Our Offices</h1>
            </div>
            <div className="line"></div>
            <div className="right">
                <div className="boxs">

                    <div className="box">
                        <video autoPlay muted loop src="./assets/video/EWKyVn3Xgo37TO7FLVqDczx7c.mp4"></video>
                        <h3>Jacob Jones</h3>
                        <pre className='pl-5'>Main Street 16</pre>
                        <pre className='pl-5'>Hamburg, Germany</pre>
                    </div>
                    <div className="box">
                        <video autoPlay muted loop src="./assets/video/6Ghd6mgGRcqS0mJGIwm4ftuIo.mp4"></video>
                         <h3>Lisbon</h3>
                        <pre className='pl-5'>Main Street 16</pre>
                        <pre className='pl-5'>Lisbon, Portugal</pre>
                    </div>
                </div>
                <div className="boxs">
                    <div className="box">
                        <video autoPlay muted loop src="./assets/video/5AhbWLTN6huw5GjliLjNaZJDWPM.mp4"></video>
                        <h3>Budapest</h3>
                        <pre className='pl-5'>Main Street 16</pre>
                        <pre className='pl-5'>Budapest, Hungary</pre>
                    </div>
                    <div className="box">
                        <video autoPlay muted loop src="./assets/video/4VtDUHNewy4UgoxJDK6CAKSJML4.mp4"></video>
                         <h3>Krakow</h3>
                        <pre className='pl-5'>Main Street 16</pre>
                        <pre className='pl-5'>Krakow, Poland</pre>
                    </div>
                </div>
            </div>
        </section>


<br />
    <hr />
    <br />
    
     <section className="view-4 mb-10">
                        <div className="left">
                            <div>
                                <p>Contact</p>
                                <h1>Let’s bring your brand to the next level</h1>
                            </div>
                            <svg viewBox="0 0 80 81" id="svg11069714695">
                                <path
                                    d="M 79.8 39.338 C 79.666 39.282 79.547 39.305 79.445 39.407 L 67.089 51.793 L 67.089 26.695 C 67.089 26.551 67.021 26.45 66.887 26.396 C 66.754 26.34 66.635 26.363 66.533 26.465 L 54.177 38.851 L 54.177 13.753 C 54.167 13.617 54.098 13.523 53.973 13.475 C 53.846 13.425 53.733 13.446 53.633 13.537 L 41.278 25.924 L 41.278 0.825 C 41.273 0.683 41.207 0.585 41.077 0.531 C 40.946 0.476 40.829 0.497 40.727 0.595 L 0.094 41.346 C -0.004 41.448 -0.025 41.565 0.029 41.695 C 0.083 41.827 0.181 41.893 0.323 41.896 L 25.345 41.896 L 13.004 54.282 C 12.902 54.384 12.879 54.503 12.933 54.637 C 12.989 54.771 13.089 54.838 13.233 54.838 L 38.251 54.838 L 25.906 67.224 C 25.804 67.328 25.781 67.445 25.835 67.579 C 25.891 67.713 25.989 67.78 26.135 67.78 L 51.157 67.78 L 38.81 80.166 C 38.71 80.27 38.685 80.387 38.741 80.521 C 38.795 80.655 38.895 80.722 39.039 80.722 L 79.674 80.722 C 79.764 80.722 79.841 80.69 79.904 80.628 C 79.968 80.563 80 80.488 80 80.396 L 80 39.652 C 80.006 39.501 79.939 39.397 79.8 39.338 Z M 1.109 41.244 L 40.633 1.613 L 40.633 26.576 L 25.995 41.244 Z M 40.633 27.485 L 40.633 41.244 L 26.915 41.244 Z M 14.015 54.188 L 26.264 41.896 L 40.959 41.896 C 41.048 41.896 41.125 41.864 41.188 41.802 C 41.252 41.737 41.284 41.659 41.284 41.57 L 41.284 26.839 L 53.539 14.549 L 53.539 39.509 L 38.906 54.188 Z M 53.539 40.429 L 53.539 54.186 L 39.821 54.186 Z M 26.919 67.126 L 39.17 54.84 L 53.865 54.84 C 53.954 54.84 54.029 54.808 54.094 54.744 C 54.156 54.681 54.19 54.604 54.19 54.514 L 54.19 39.777 L 66.443 27.485 L 66.443 52.445 L 51.812 67.126 Z M 66.443 53.369 L 66.443 67.126 L 52.727 67.126 Z M 79.349 80.064 L 39.831 80.064 L 52.08 67.778 L 66.768 67.778 C 66.858 67.778 66.935 67.746 67 67.682 C 67.062 67.619 67.094 67.542 67.094 67.452 L 67.094 52.717 L 79.349 40.429 Z"
                                    fill="var(--token-efbc6c56-80fe-4475-9b37-9684d6e92632, rgb(25, 25, 25))"></path>
                            </svg>
                        </div>
                        <div className="line"></div>
                        <div className="right">
                            <h3>Together, let's elevate your brand to new heights by unleashing its full potential and captivating your target audience.</h3>
                            <button>Get in Touch</button>
                        </div>
                    </section>
    
                    <hr />
                    <br />
                    <Footer />

            </main>
    </>
  )
}

export default Contact